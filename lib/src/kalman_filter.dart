...
class KalmanFilter {
  static Location apply(Location location, Location? lastLocation, double constant) {
    ...
    if (lastLocation != null) {
      final timestampInc =
          location.time.microsecondsSinceEpoch - lastLocation.time.microsecondsSinceEpoch;

      if (timestampInc > 0) {
        // We can tune the velocity and particularly the coefficient at the end
        final speed = SphericalUtil.computeDistanceBetween(location.latlng, lastLocation.latlng) /
            timestampInc *
            constant;
